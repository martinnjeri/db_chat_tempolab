...
export default function DatabaseTest() {
	...
	async function testTables() {
		...
		try {
			...
			const { data: functionTest, error: functionError } =
			...
			if (functionError) {
				...
				throw new Error(
					`execute_sql function error: ${functionError.message}`
				);
			}
        ...
        `,
			...
			const tableData = await Promise.all(
				tables.map(async (table: { name: string }) => {
					try {
						const { data: sampleData, error: sampleError } =
							await supabase.rpc("execute_sql", {
					...
					{error && (
						<div className="p-4 bg-destructive/10 text-destructive rounded-md text-sm">
							...
							<p className="text-xs text-muted-foreground mt-2">
								...
								your Supabase credentials are correct in
								.env.local and that the database has the
								execute_sql function properly configured.
							</p>
							<p className="text-xs text-muted-foreground mt-1">
								The error might be related to multiple versions
								of the execute_sql function. Make sure all calls
								include the org_id parameter.
							</p>
						</div>
					)}
