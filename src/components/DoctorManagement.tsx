...
export default function DoctorManagement() {
		...
		try {
			...
			const { data, error } = await supabase
				...
				.insert({
					...
					Specialty: doctorSpecialty.trim(), // Note the capital 'S' to match the database schema
					contact_number: doctorContact.trim() || null,
					organization_id:
						doctorOrgId !== "none"
							? parseInt(doctorOrgId, 10)
							: null,
				})
				.select();
	...
	const openEditDialog = (doctor: Doctor) => {
		...
		setDoctorSpecialty(doctor.Specialty); // Note the capital 'S' to match the database schema
		...
		<div className="space-y-6">
			...
			{loading ? (
				...
				<Card>
					...
					<CardFooter>
						<Button onClick={openCreateDialog}>
							<PlusCircle className="mr-2 h-4 w-4" /> Add Your
							First Doctor
						</Button>
					</CardFooter>
				</Card>
					...
					<TableBody>
						{doctors.map((doctor) => (
							<TableRow key={doctor.id}>
								...
								{/* Note the capital 'S' to match the database schema */}
								<TableCell>
